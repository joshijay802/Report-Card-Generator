...
# from LoginPage import StudentLoginform



def StudentRegistrationForm():
    root=Tk()
    root.title("Student's Form")
    def center_screen():
        window_height = 600
        window_width = 600
    ...
    center_screen()
    Heading=Label(root,text="Student's Registration", font=('Arial',17),bg='orange',fg='white')
    Heading.place(x=0,y=10, width=600, height=50)

    enrollment=Label(root,text="Enrollment no. : ")
    enrollment_data = StringVar()
    enrollment_data=Entry(root,textvariable=enrollment_data)
    enrollment.place(x=130 ,y=100 )
    enrollment_data.place(x=250 ,y=100 ,width=200)
    ...
    def clearVal():
        enrollment_data.delete(0,END)
        enrollment_data.insert(0, "")
    ...
    def submitDetails():
        student_details=[enrollment_data.get(),fname_data.get(),lname_data.get(),email_data.get(),
                        branch_data.get(),contact_data.get(),sem_data.get(),
                        school_data.get(),password_data.get()]
        
        try:
            RegisterStudent(student_details)
            clearVal()
            root.destroy()
            # StudentLoginform()
